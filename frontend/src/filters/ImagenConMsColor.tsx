// src/filters/ImagenConMsColor.tsx
import React, { useState } from 'react';

interface ImagenConMsColorProps {
  selectedImage: File | null;
  setProcessedImageUrl: (url: string) => void;
  setIsProcessing: (isProcessing: boolean) => void;
}

const ImagenConMsColor: React.FC<ImagenConMsColorProps> = ({
  selectedImage,
  ...
  const applyFilter = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5000/apply-letras-m-color', {
        method: 'POST',
        body: formData,
      });
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Error al aplicar el filtro LetrasMColor');
      }
      ...
      console.error('Error al aplicar el filtro LetrasMColor:', error);
  ...
  return (
    <div>
      <h3>Aplicar Filtro Letras M en Color</h3>
      <label>
      ...
      <br />
      <button onClick={applyFilter}>Aplicar Filtro Letras M en Color</button>
      ...
      {htmlUrl && (
        <div className="html-preview">
          ...
          <a href={htmlUrl} target="_blank" rel="noopener noreferrer">
            Ver Imagen en M's Color
          </a>
          <br />
          {/* Opcional: Mostrar el HTML dentro de un iframe */}
          <iframe
            src={htmlUrl}
            title="Imagen en M's Color"
...
export default ImagenConMsColor;
