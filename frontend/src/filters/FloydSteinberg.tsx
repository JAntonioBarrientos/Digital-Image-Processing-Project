...
interface FloydSteinbergDitheringProps {
...
const FloydSteinbergDithering: React.FC<FloydSteinbergDitheringProps> = ({ selectedImage, setImagePreview, setProcessedImageUrl, setIsProcessing }) => {
  ...
  const applyFloydSteinbergDithering = async () => {
    if (!selectedImage) {
    ...
    try {
      const response = await fetch('http://localhost:5000/apply-floyd-steinberg-dithering', {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div>
      ...
      <button onClick={applyFloydSteinbergDithering}>Aplicar Dithering Floyd Steinberg</button>
    </div>
  );
};
